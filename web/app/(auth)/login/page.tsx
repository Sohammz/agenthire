...
export default function LoginPage() {
  ...
  // 🔹 Magic link login (and redirect new users)
  const signInWithEmail = async () => {
    ...
    try {
      ...
      const { data, error: listErr } = await supabase.auth.admin.listUsers(); // requires service role in backend only — fallback to try sign in
      if (listErr) console.warn('listUsers unavailable, fallback to sign in');

      // try sending OTP (if account exists)
      ...
      if (error) {
        // Supabase doesn't expose 'user not found' directly
        setMsg('Redirecting to signup…');
        setTimeout(() => router.push(`/signup?email=${encodeURIComponent(email)}`), 800);
      } else {
        setMsg('Check your email for the sign-in link.');
      }
    } catch (err: any) {
      console.error(err);
      setMsg('Could not send sign-in link. Redirecting to signup…');
      setTimeout(() => router.push(`/signup?email=${encodeURIComponent(email)}`), 800);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-6 py-20">
        <motion.section initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          {/* LEFT: OAuth sign in */}
          <motion.div whileHover={{ scale: 1.01 }} className="p-6 rounded-2xl bg-gradient-to-br from-white/80 to-white/60 dark:from-gray-900/60 dark:to-gray-900/40 shadow-lg flex flex-col justify-top gap-6">
            <div>
              <h3 className="text-xl font-semibold">Sign in quickly</h3>
              <p className="text-sm text-gray-500 mt-1">Use Google or GitHub to sign in instantly.</p>
            </div>

            <div className="flex flex-col gap-3">
              <motion.button whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }} onClick={() => signInWithOAuth('google')} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-white border shadow-sm hover:shadow-md transition">
              ...
              <motion.button whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }} onClick={() => signInWithOAuth('github')} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-gray-50 dark:bg-gray-800 border shadow-sm hover:shadow-md transition">
          ...
          <motion.div whileHover={{ scale: 1.01 }} className="p-6 rounded-2xl bg-gradient-to-br from-white/70 to-white/50 dark:from-gray-900/60 dark:to-gray-900/40 shadow-lg flex flex-col justify-between gap-4">
            <div>
              <h3 className="text-xl font-semibold">Sign in with email</h3>
              <p className="text-sm text-gray-500 mt-1">Enter your registered email to get a magic link.</p>
            </div>
            ...
            <div className="grid gap-3 mt-4">
              ...
              <input
                ...
                type="email"
                className="w-full px-4 py-3 rounded-lg bg-white/90 dark:bg-gray-800/60 focus:outline-none focus:ring-2 focus:ring-purple-400 transition"
              />
              ...
              <motion.button
                ...
                disabled={loading}
                className={`flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-white font-medium shadow-md transition ${loading ? 'bg-purple-500 cursor-wait' : 'bg-purple-600 hover:bg-purple-700'}`}
              >
                <Mail size={16} />
            ...
            <div className="text-sm text-gray-600 mt-3">
              New here?{' '}
              <a href="/signup" className="underline text-purple-600 hover:text-purple-700">
                Create an account
              </a>
            </div>
          </motion.div>
